...
@Component({
  selector: 'app-vehicles-clean',
  standalone: true,
  imports: [FormsModule, RouterLink],
  template: `
    <main class="page">
      <header class="topbar">
        <a class="logo" routerLink="/dashboard"><b>PC</b> PROJECT COST</a>
        <nav>
      ...
      <section class="content">
        <div class="filters">
          <label>Matricule<input [(ngModel)]="filters['matricule']" placeholder="Rechercher" /></label>
          <label>Visite technique<input [(ngModel)]="filters['visiteTechnique']" placeholder="Rechercher" /></label>
          <label>Assurance<input [(ngModel)]="filters['assurance']" placeholder="Rechercher" /></label>
          <label>Taxe<input [(ngModel)]="filters['taxe']" placeholder="Rechercher" /></label>
        </div>
          ...
          <table>
            ...
            <tbody>
              @for (vehicle of filteredVehicles; track vehicle.id) {
                <tr>
                  <td><strong>{{ vehicle.matricule || '-' }}</strong></td>
                  <td>{{ vehicle.visiteTechnique || '-' }}</td>
                  <td>{{ vehicle.assurance || '-' }}</td>
                  <td>{{ vehicle.taxe || '-' }}</td>
                  <td class="row-actions">
                    <button type="button" (click)="openEditModal(vehicle)">Modifier</button>
                    <button type="button" class="danger" (click)="deleteVehicle(vehicle.id)">Supprimer</button>
                  </td>
                </tr>
              } @empty {
              ...
              <tbody>
                @for (expense of expenses; track expense.id) {
                  <tr>
                    ...
                    <td>{{ expense.prochainKilometrage ?? '-' }}</td>
                    <td class="row-actions">
                  ...
                  <tr><td colspan="6" class="empty">Aucune dépense enregistrée.</td></tr>
                }
              </tbody>
            </table>
          </div>
        ...
        <div class="modal-backdrop">
          <form class="modal" (ngSubmit)="saveVehicle()">
            ...
            <div class="fields">
              ...
              <label>Prochain km vidange<input name="prochainKilometrageVidange" type="number" min="0" [(ngModel)]="draft.prochainKilometrageVidange" /></label>
            </div>
            <div class="actions">
        ...
        <div class="modal-backdrop">
          <form class="modal" (ngSubmit)="saveExpense()">
            ...
            <div class="fields">
              ...
              <label>Prochain kilométrage<input name="prochainKilometrage" type="number" min="0" [(ngModel)]="expenseDraft.prochainKilometrage" /></label>
            </div>
            <div class="actions">
  ...
  styles: [`
    ...
    .filters { display:grid; grid-template-columns:repeat(4, minmax(160px, 1fr)); gap:12px; margin-bottom:18px; }
...
export class VehiclesCleanComponent implements OnInit {
  ...
  filters: Record<string, string> = { matricule: '', visiteTechnique: '', assurance: '', taxe: '' };
  editingId = '';
  isModalOpen = false;
  ...
  get filteredVehicles(): Vehicule[] {
    return this.vehicles.filter((vehicle) => {
      const m = this.filters['matricule'] ?? '';
      const vis = this.filters['visiteTechnique'] ?? '';
      const ass = this.filters['assurance'] ?? '';
      const tax = this.filters['taxe'] ?? '';

      return (!m || (vehicle.matricule ?? '').toLowerCase().includes(m.toLowerCase()))
        && (!vis || (vehicle.visiteTechnique ?? '').toLowerCase().includes(vis.toLowerCase()))
        && (!ass || (vehicle.assurance ?? '').toLowerCase().includes(ass.toLowerCase()))
        && (!tax || (vehicle.taxe ?? '').toLowerCase().includes(tax.toLowerCase()));
    });
  }
  ...
  saveVehicle(): void {
    const vehicle = { ...this.draft, id: this.editingId || this.makeId() };
  ...
  openCreateExpenseModal(category = 'reparation'): void {
    ...
    this.expenseDraft.vehiculeId = this.vehicles.length ? this.vehicles[0].id : '';
  ...
  saveExpense(): void {
    ...
    if (!this.expenseDraft.montant || this.expenseDraft.montant <= 0) {
      this.message = 'Le montant doit être supérieur à zéro.';
      return;
    }

    const expense = { ...this.expenseDraft, id: this.editingExpenseId || this.makeId() };
  ...
  getVehicleMatricule(vehicleId: string): string {
    ...
    return vehicle?.matricule ?? 'Non renseigné';
  }

  private emptyVehicle(): Vehicule {
    return { id: '', matricule: '', visiteTechnique: '', assurance: '', taxe: '', prochainKilometrageVidange: 0 };
  }

  private emptyExpense(category = 'reparation'): VehiculeFrais {
    return { id: '', vehiculeId: this.vehicles.length ? this.vehicles[0].id : '', montant: 0, description: '', categorie: category, prochainKilometrage: 0, kilometrage: 0 };
  }
